...
@dataclass
class Server:
	socket_address: socket._Address
	...
	def start_accepting_connections(self, thread_args=dict(daemon=True)):
		self.accept_connections_thread = threading.Thread(target=self.accept_connections, **thread_args)
	...
	def clients_remove_disconnected(self):
		for index in reversed(self.disconnected_indices):
			logger.info(f'client[{index}] disconnected and removed {self.clients[index]}')
	...
	def accept_connections(
		...
		while True:
			...
			client = Client(client_socket=client_socket, data_queue=Queue(), address=address)
			self.clients.append(client)
			client.thread = threading.Thread(target=client.handler, args=(), daemon=True)
			client.thread.start()
